...
function analysisObject(n) {
  ...
  if (n instanceof Model) {
    ...
    let isNotSettingKeys = keys.some(item => [
      'type',
      'default',
      'unit',
      'format',
...
function parseObject(data, model, param, parent) {
  ...
  switch (model.type) {
    case TYPE.OBJECT:
      ...
      const columns = 0;
      if (param.isParse) {
        const keys = Utils.mergeArray(Object.keys(model.value), data
          ? Object.keys(data)
          : []);
        for (const i of keys) {
          if (model.value.hasOwnProperty(i)) {
            data = data || {};
            const _out = parseObject(model.value[i].property
              ? getDataFromPath(data, model.value[i].property)
              : data[i], model.value[i], param, data);
      ...
      // 依旧为空对象
      if (Object.keys(outData).length == 0 && param.removeEmptyObject && !Utils.isArray(parent)) 
        outData = null;
      break;
    case TYPE.ARRAY:
      outData = [];
...
function getDataFromPath(data = {}, path) {
  let keys = utils.isArray(path)
    ...
    index = path.length,
    outData = '';
  let parent = data[keys[0]];
  if (utils.isObject(parent)) {
    outData = getDataFromPath(parent, keys.splice(0, 1) && keys)
  } else {
    outData = parent
  }
  return outData
